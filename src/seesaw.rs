//! Async Driver for the Adafruit Seesaw.
...
where
    I2C: I2c,
{
    async fn write(&mut self, base: u8, function: u8, buf: &[u8]) -> Result<(), Error> {
    ...
    async fn read(&mut self, base: u8, function: u8, buf: &mut [u8]) -> Result<(), Error> {
        self.write(base, function, &[]).await?;
        Timer::after(Duration::from_micros(1000));
        self.i2c
    ...
    pub async fn keypad_get_count(&mut self) -> Result<u8, Error> {
        let mut buf = [0u8; 1];
        self.read(keypad::BASE, keypad::functions::COUNT, &mut buf)
            .await?;
        Ok(buf[0])
    }

    /// Enable or disable the interrupt
    pub async fn keypad_set_interrupt(&mut self, enable: bool) -> Result<(), Error> {
        use keypad::functions::{INTENCLR, INTENSET};

        let func = if enable { INTENSET } else { INTENCLR };
        self.write(keypad::BASE, func, &[1]).await
    }
    ...
    /// actual u8 values
    pub async fn keypad_set_event_raw(
        &mut self,
        key_raw: u8,
        edge: keypad::Edge,
    ...
    pub async fn keypad_read_raw(&mut self, buf: &mut [u8]) -> Result<(), Error> {
        self.read(keypad::BASE, keypad::functions::FIFO, buf).await
    }

    pub async fn neopixel_set_pin(&mut self, pin: u8) -> Result<(), Error> {
    ...
    pub async fn neopixel_set_speed(&mut self, speed: neopixel::Speed) -> Result<(), Error> {
        self.write(neopixel::BASE, neopixel::functions::SPEED, &[speed as u8])
            .await
    }

    pub async fn neopixel_set_buf_length_bytes(&mut self, len: u16) -> Result<(), Error> {
    ...
    pub async fn neopixel_set_buf_length_pixels(
        &mut self,
        ct: usize,
        ...
        if count <= (u16::max_value() as usize) {
            self.neopixel_set_buf_length_bytes(count as u16).await
        } else {
            Err(Error::SeeSaw(SeeSawError::InvalidArgument))
        }
    }

    pub async fn neopixel_show(&mut self) -> Result<(), Error> {
        self.write(neopixel::BASE, neopixel::functions::SHOW, &[])
            .await
    }

    pub async fn neopixel_write_buf_raw(&mut self, idx: u16, buf: &[u8]) -> Result<(), Error> {
        ...
        self.write(
            neopixel::BASE,
            neopixel::functions::BUF,
            &tx_buf[..tx_buf_len],
        )
        .await
    }

    pub async fn status_get_hwid(&mut self) -> Result<u8, Error> {
        let mut buf = [0u8; 1];
    ...
    pub async fn status_get_version(&mut self) -> Result<u32, Error> {
        let mut buf = [0u8; 4];
    ...
    pub async fn status_get_options(&mut self) -> Result<u32, Error> {
        let mut buf = [0u8; 4];
    ...
    pub async fn status_get_temp_raw(&mut self) -> Result<u32, Error> {
        let mut buf = [0u8; 4];
